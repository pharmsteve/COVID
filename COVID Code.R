...
#Authors: Stephen Chukwuma Ogbodo, Omotayo Olaoye, and Joseph Junior Damba
#Happy coding! :)
...
map1<-
  ...
  theme_dark()
map1
ggsave("fig4.jpg", units="in", width=10, height=7, dpi=300)
...
table1 <- tbl_summary(dat5, by=vac, missing="no", digits=all_continuous() ~ 0,
                      percent = "row",
...
mod1a <- glmmTMB(cases  ~ vacc_l + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string_l + gdp + life + tests + beds + Continent + month + (1 | location), 
                 data = dat5, 
                 family = nbinom2)
tbl_regression(mod1a, exponentiate = T)

#Categorical Exposure Variable
mod1b <- glmmTMB(cases  ~ vac_l + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string_l + gdp + life + tests + beds + Continent + month + (1 | location), 
                           data = dat5, 
                           family = nbinom2)
...
#Continuous Exposure Variable
mod1c <- glmmTMB(deaths  ~ vacc_l + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string_l + gdp + life + tests + beds + Continent + month + (1 | location), 
                 data = dat5, 
                 family = nbinom2)
tbl_regression(mod1c, exponentiate = T)

#Categorical Exposure Variable
mod1d <- glmmTMB(deaths  ~ vac_l + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string_l + gdp + life + tests + beds + Continent + month + (1 | location), 
                           data = dat5, 
                           family = nbinom2)
...
models %>% modelplot(coef_omit = "Interc|Cont|bed|age|test|life|gdp|strin|diab|card|pop|loc|month",
  ...
  My_Theme
ggsave("fig5.jpg", units="in", width=10, height=7, dpi=300)
...
dat3 <- dat2 %>%
  ...
  mutate(new_vacc = na.approx(fully_vac, na.rm = F)) |> #Fill in NA by linear interpolation
  fill(new_vacc, .direction = "down")|> #Replace trailing NA's with the preceding value
  mutate(string = case_when(time %in% c(13, 14, 15, 16) ~ 0, .default = strin)) |> #Assign zero stringency to the last month
...
##OUTCOME: COVID-19 CASES
#Continuous Exposure Variable
mod1a <- glmmTMB(cases  ~ new_vacc + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string + gdp + life + tests + beds + Continent + month + (1 | location), 
                 data = dat5, 
                 family = nbinom2)
tbl_regression(mod1a, exponentiate = T)

#Categorical Exposure Variable
mod1b <- glmmTMB(cases  ~ vac + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string + gdp + life + tests + beds + Continent + month + (1 | location), 
                 data = dat5, 
                 family = nbinom2)
tbl_regression(mod1b, exponentiate = T)
...
##OUTCOME: COVID-19 DEATHS
#Continuous Exposure Variable
mod1c <- glmmTMB(deaths  ~ new_vacc + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string + gdp + life + tests + beds + Continent + month + (1 | location), 
                 data = dat5, 
                 family = nbinom2)
tbl_regression(mod1c, exponentiate = T)

#Categorical Exposure Variable
mod1d <- glmmTMB(deaths  ~ vac + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string_l + gdp + life + tests + beds + Continent + month + (1 | location), 
                 data = dat5, 
                 family = nbinom2)
tbl_regression(mod1d, exponentiate = T)
...
##OUTCOME: COVID-19 CASES
#Continuous Exposure Variable
mod1a <- glmmTMB(cases  ~ vacc_l2 + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string_l2 + gdp + life + tests + beds + Continent + month + (1 | location), 
                 data = dat5, 
                 family = nbinom2)
tbl_regression(mod1a, exponentiate = T)

#Categorical Exposure Variable
mod1b <- glmmTMB(cases  ~ vac_l2 + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string_l2 + gdp + life + tests + beds + Continent + month + (1 | location), 
                 data = dat5, 
                 family = nbinom2)
tbl_regression(mod1b, exponentiate = T)
...
##OUTCOME: COVID-19 DEATHS
#Continuous Exposure Variable
mod1c <- glmmTMB(deaths  ~ vacc_l2 + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string_l2 + gdp + life + tests + beds + Continent + month + (1 | location), 
                 data = dat5, 
                 family = nbinom2)
tbl_regression(mod1c, exponentiate = T)

#Categorical Exposure Variable
mod1d <- glmmTMB(deaths  ~ vac_l2 + offset(log(pop)) + pop_den + card + diab + aged_65 + med_age + string_l2 + gdp + life + tests + beds + Continent + month + (1 | location), 
                 data = dat5, 
                 family = nbinom2)
tbl_regression(mod1d, exponentiate = T)
